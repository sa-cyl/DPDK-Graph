...
#ifndef CE_Graph_CONVERSIONS_DEF
#define CE_Graph_CONVERSIONS_DEF
...
namespace CE_Graph {
    ...
    void convert_edgelist(std::string inputfile, sharder<EdgeDataType, FinalEdgeDataType> &sharderobj, bool multivalue_edges=false) {
        ...
        while(fgets(s, 1024, inf) != NULL) {
            linenum++;
            if ( (linenum & 0x7FFFFF) == 0) {
                logstream(LOG_DEBUG) << "Read " << (linenum >> 20) << " Mega lines, " << bytesread / 1024 / 1024.  << " MB" << std::endl;
            }
            FIXLINE(s);
            bytesread += strlen(s);
    ...
    int convert(std::string basefilename, std::string nshards_string) {
		...
		sprintf(infofilename , "%s%d.shoveled" , basefilename.c_str() , sizeof(EdgeDataType));
		FILE * fp = fopen(infofilename , "r");
		if( fp == NULL )
		{
		
			std::string file_type_str = get_option_string_interactive("filetype", "edgelist, adjlist, binedgelist, metis , multivalueedgelist(Only for dynamic edges)");
		...
		{
			logstream(LOG_INFO) << "Detected info files , skip shoveling..." << std::endl;
			int numshovels;
			long max_vertex_id , shoveled_edges;
			fscanf(fp , "%d\n%ld\n%ld" , &numshovels , &max_vertex_id , &shoveled_edges);
			sharderobj.set_skip_shovel(numshovels , max_vertex_id , shoveled_edges);
			fclose(fp);
		}
    ...
    static int VARIABLE_IS_NOT_USED convert_none(std::string basefilename, std::string nshards_string) {
		...
		std::string infofilename = basefilename + "1.shoveled";
