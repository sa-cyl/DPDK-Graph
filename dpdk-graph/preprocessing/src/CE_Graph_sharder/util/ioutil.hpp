...
template <typename T>
ssize_t reada(int f, T * tbuf, size_t nbytes) {
    size_t nread = 0;
    char * buf = (char*)tbuf;
    while(nread<nbytes) {
        ssize_t a = read(f, buf, nbytes - nread);
        if (a == (-1)) {
            logstream(LOG_ERROR) << "Error, could not read: " << strerror(errno) << "; file-desc: " << f << std::endl;
            logstream(LOG_ERROR) << "reada arguments: " << f << " tbuf: " << tbuf << " nbytes: " << nbytes << std::endl;
            if(errno == EAGAIN)
			{
				break;
			}
