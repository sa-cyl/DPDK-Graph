 ...
 * @section DESCRIPTION
 *
 * Returns standard filenames for all the data files used by CE_Graph.
 * All functions expect a "basefilename".
 * You can specify environment variable "CE_Graph_ROOT", which is the
 * root directory for the CE_Graph configuration and source directories.
 */

#ifndef CE_Graph_FILENAMES_DEF
#define CE_Graph_FILENAMES_DEF
...
#define CE_Graph_DISABLE_COMPRESSION
    ...
    static std::string filename_config() {
        char * chi_root = getenv("CE_Graph_ROOT");
        if (chi_root != NULL) {
            return std::string(chi_root) + "/conf/CE_Graph.cnf";
        } else {
            return "conf/CE_Graph.cnf";
        }
    }
    ...
    static std::string filename_config_local() {
        char * chi_root = getenv("CE_Graph_ROOT");
        if (chi_root != NULL) {
            return std::string(chi_root) + "/conf/CE_Graph.local.cnf";
        } else {
            return "conf/CE_Graph.local.cnf";
        }
    }
	...
	static std::string dirname_shard(std::string basefilename , int p , int nshards)
	{
		std::stringstream ss;
        ss << basefilename;
        ss << ".subgraph.";
