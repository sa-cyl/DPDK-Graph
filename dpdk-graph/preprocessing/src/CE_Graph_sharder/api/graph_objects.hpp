...
#define VARIABLE_IS_NOT_USED
    ...
    public:
        ...
        CE_Graph_edge() {}
        CE_Graph_edge(vid_t _vertexid, EdgeDataType * edata_ptr) : vertexid(_vertexid), data_ptr(edata_ptr) {
        }
    ...
    public:
...
#endif
            if (inedges_ptr != NULL) 
                inedges_ptr[inc] = CE_Graph_edge<EdgeDataType>(src, ptr);
        ...
        inline void add_outedge(vid_t dst, EdgeDataType * ptr, bool special_edge) {
            ...
            if (outedges_ptr != NULL) outedges_ptr[i-1] = CE_Graph_edge<EdgeDataType>(dst, ptr);
            assert(dst != vertexid);
        }
