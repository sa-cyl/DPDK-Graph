 ...
 */
#ifndef CE_Graph_CONFIGFILE_DEF
#define CE_Graph_CONFIGFILE_DEF
    ...
    static std::map<std::string, std::string> loadconfig(std::string filename, std::string secondary_filename) {
        ...
        if (f == NULL) {
            ...
            if (f == NULL) {
                ...
                std::cout << "Please define environment variable CE_Graph_ROOT or run the program from that directory." << std::endl;
            }
            assert(f != NULL);
        }
