 ...
 * @section DESCRIPTION
 *
 * Alternative "functional" API for CE_Graph. The API is implemented as a 
...
#ifndef CE_Graph_FUNCTIONALAPI_DEF
#define CE_Graph_FUNCTIONALAPI_DEF
    ...
    /**
     ...
     struct functional_kernel {
        ...
        /* Initial value - on first iteration */
         virtual VertexDataType initial_value(CE_Graph_context &info, vertex_info& myvertex) = 0;        
        /* Called before first "gather" */
         virtual VertexDataType reset()  = 0;        
        // Note: Unweighted version, edge value should also be passed
        // "Gather"
        virtual EdgeDataType op_neighborval(CE_Graph_context &info, vertex_info& myvertex, vid_t nbid, EdgeDataType nbval)= 0;         
        // "Sum"
        virtual EdgeDataType plus(VertexDataType curval, EdgeDataType toadd) = 0;
         // "Apply"
        virtual VertexDataType compute_vertexvalue(CE_Graph_context &ginfo, vertex_info& myvertex, EdgeDataType nbvalsum) = 0;        
        // "Scatter
        virtual EdgeDataType value_to_neighbor(CE_Graph_context &info, vertex_info& myvertex, vid_t nbid, VertexDataType myval) = 0;        
    }; 
