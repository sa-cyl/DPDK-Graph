...
struct smoketest_program : public functional_kernel<int, int> {
    ...
    int initial_value(CE_Graph_context &info, vertex_info& myvertex) {
        return 0;
    }
    ...
    // "Gather"
    int op_neighborval(CE_Graph_context &info, vertex_info& myvertex, vid_t nbid, int nbval) {
    ...
    // "Apply"
    int compute_vertexvalue(CE_Graph_context &ginfo, vertex_info& myvertex, int nbvalsum) {
        return ginfo.iteration;
    }
    
    // "Scatter
    int value_to_neighbor(CE_Graph_context &info, vertex_info& myvertex, vid_t nbid, int myval) {
