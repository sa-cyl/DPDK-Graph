 ...
 * @section DESCRIPTION
 *
 * Simple smoketest for the dynamic graph CE_Graph engine.
 */
 ...
 */
struct SmokeTestProgram2 : public CE_GraphProgram<VertexDataType, EdgeDataType> {
    ...
    void update(CE_Graph_vertex<VertexDataType, EdgeDataType> &vertex, CE_Graph_context &gcontext) {
        ...
        if (gcontext.iteration == 0) {
            ...
            for(int i=0; i < vertex.num_outedges(); i++) {
                CE_Graph_edge<vid_t> * edge = vertex.outedge(i);
...
int main(int argc, const char ** argv) {
    ...
    CE_Graph_dynamicgraph_engine<VertexDataType, EdgeDataType> engine(filename, nshards, scheduler, m); 
