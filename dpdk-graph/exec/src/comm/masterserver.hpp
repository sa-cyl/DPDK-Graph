...
static void *reply(void *rc){
	...
	int *li;
	int fd=(*(RC*)rc).fd;
	int I=(*(runcontrol*)((*(RC*)rc).R)).intervals;
	task *T=(*(runcontrol*)((*(RC*)rc).R)).T;
	...
	if(Pack_t[0]=='A'){
		...
		(*(runcontrol*)((*(RC*)rc).R)).windowpos += 1;	
		int n = (*(runcontrol*)((*(RC*)rc).R)).windowpos;
		if(n < I){
			send(T[n].machine, (char*)"XGACKN", 6, 0);	
		}
	}else{
