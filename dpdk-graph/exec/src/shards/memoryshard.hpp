...
#pragma omp parallel for
    ...
    private:
        ...
        void load_edata_XG() {
             ...
             for(int i = 0; i < P; i++){
                 ...
                 if (file_exists(block_filename)) {
                    ...
                    if(isload){
                        ...
                        inlength[i] = fsize;
			if(i==exec_interval){ //for synchronization
	     ...
	     memcpy(main_to_main_buf, inbuf[exec_interval], inlength[exec_interval]); //for synchronization
    } 
    public:
...
#pragma omp parallel for
	    ...
	    int edgeptr_syn=0;
			...
			if(asyn == 0){
				__sync_add_and_fetch(&nedges, 1);	
                        	vertex->add_outedge(target, (only_adjacency ? NULL : (ET*) eptr), (ET*)(main_to_main_buf + edgeptr_syn) , false);
			} else{
				__sync_add_and_fetch(&nedges, 1);	
                        	vertex->add_outedge(target, (only_adjacency ? NULL : (ET*) eptr), NULL , false);
			}
                    }
				    ...
				    if((vid<=window_en)&&(vid>=window_st)&&(asyn==0)){
					//Add asyn pionter.
					__sync_add_and_fetch(&nedges, 1);
                                    	dstvertex.add_inedge(vid,  (only_adjacency ? NULL : (ET*) eptr), (ET*)(main_to_main_buf + edgeptr_syn) , false);  //Add a syn pointer
				    }else{
					__sync_add_and_fetch(&nedges, 1);
                                    	dstvertex.add_inedge(vid,  (only_adjacency ? NULL : (ET*) eptr), NULL, false);  //Add a syn pointer
				    }
                    ...
                    edgeptr += sizeof(ET);
		    if((vid<=window_en) && ( vid>=window_st)) edgeptr_syn += sizeof(ET);
                    if(edgeptr>=brange) {
                        bid++;
                        brange += blocksizes[bid];
                    } 
