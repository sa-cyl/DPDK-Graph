...
#ifndef DEF_CE_Graph_SLIDINGSHARD
#define DEF_CE_Graph_SLIDINGSHARD
...
namespace CE_Graph {
     ...
     * a time.
     */
    template <typename VT, typename ET, typename svertex_t = CE_Graph_vertex<VT, ET>, typename ETspecial = ET>
    class sliding_shard {
                ...
                if (vertex.scheduled){
                    while(--n >= 0) {
		       	    ...
		       	    __sync_add_and_fetch(&nedges, 1);
                            vertex.add_outedge(target, evalue, NULL, special_edge);
