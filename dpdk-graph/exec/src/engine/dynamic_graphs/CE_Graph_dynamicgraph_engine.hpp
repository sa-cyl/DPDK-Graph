...
#ifndef CE_Graph_DYNAMICGRAPHENGINE_DEF
#define CE_Graph_DYNAMICGRAPHENGINE_DEF

#include <stdlib.h>
#include <vector>

#include "engine/CE_Graph_engine.hpp"
...
namespace CE_Graph {
    ...
    template <typename VertexDataType, typename EdgeDataType, typename svertex_t = CE_Graph_vertex<VertexDataType, EdgeDataType> >
    class CE_Graph_dynamicgraph_engine : public CE_Graph_engine<VertexDataType, EdgeDataType, svertex_t> {
    public:
        typedef CE_Graph_engine<VertexDataType, EdgeDataType>  base_engine;
        typedef edge_buffer_flat<EdgeDataType> edge_buffer; 
        
        CE_Graph_dynamicgraph_engine(std::string base_filename, int nshards, bool selective_scheduling, metrics &_m) :
        CE_Graph_engine<VertexDataType, EdgeDataType, svertex_t>(base_filename, nshards, selective_scheduling, _m){
        ...
        virtual vid_t determine_next_window(vid_t iinterval, vid_t fromvid, vid_t maxvid, size_t membudget) {
            ...
            for(int i=0; i < max_interval; i++) {
                ...
                memreq += sizeof(svertex_t) + (sizeof(EdgeDataType) + sizeof(vid_t) + 
                                               sizeof(CE_Graph_edge<EdgeDataType>))*(outc + inc);
                if (memreq > membudget) {
...
#define BBUF 32000000
    ...
    public:
        std::string get_info_json() {
            ...
            /**
              * FIXME: too much duplicate with CE_Graph_engine
              */
            json << "{";
            json << "\"state\" : \"" << state << "\",\n";
