#ifndef __CE_Graph_RMSE_ENGINE
#define __CE_Graph_RMSE_ENGINE
/**
 * @file
 ...
 */
struct ValidationAPProgram : public CE_GraphProgram<VertexDataType, EdgeDataType> {
 ...
 * class. The main logic is usually in the update function.
 */
struct ValidationRMSEProgram : public CE_GraphProgram<VertexDataType, EdgeDataType> {
...
void init_validation_rmse_engine(CE_Graph_engine<VertexDataType,EdgeDataType> *& pvalidation_engine, int nshards,float (*prediction_func)(const vertex_data & user, const vertex_data & movie, float rating, double & prediction, void * extra)){
  if (nshards == -1)
    return;
  metrics * m = new metrics("validation_rmse_engine");
  CE_Graph_engine<VertexDataType, EdgeDataType> * engine = new CE_Graph_engine<VertexDataType, EdgeDataType>(validation, nshards, false, *m); 
...
template<typename VertexDataType, typename EdgeDataType>
void run_validation(CE_Graph_engine<VertexDataType, EdgeDataType> * pvalidation_engine, CE_Graph_context & context){
