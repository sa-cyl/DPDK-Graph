...
/**
  ...
  */
void test_predictions(float (*prediction_func)(const vertex_data & user, const vertex_data & movie, float rating, double & prediction, void * extra), CE_Graph_context * gcontext = NULL, bool dosave = true, vec * avgprd = NULL, int pmf_burn_in = 0) {
  MM_typecode matcode;
  FILE *f;
  uint Me, Ne;
  size_t nz;   
...
void validation_rmse(float (*prediction_func)(const vertex_data & user, const vertex_data & movie, float rating, double & prediction, void * extra)
    ,CE_Graph_context & gcontext, int tokens_per_row = 3, vec * avgprd = NULL, int pmf_burn_in = 0) {
  FILE *f;
  size_t nz;   
...
void validation_rmse3(float (*prediction_func)(const vertex_data & user, const vertex_data & movie, const vertex_data & time, float rating, double & prediction)
    ,CE_Graph_context & gcontext,int tokens_per_row = 4, int time_offset = 0) {
  MM_typecode matcode;
  FILE *f;
  size_t nz;   
...
double training_rmse(int iteration, CE_Graph_context &gcontext, bool items = false){
