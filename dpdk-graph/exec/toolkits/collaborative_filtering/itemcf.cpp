...
#include "engine/dynamic_graphs/CE_Graph_dynamicgraph_engine.hpp"
...
class adjlist_container {
   ...
   */
  int load_edges_into_memory(CE_Graph_vertex<uint32_t, uint32_t> &v) {
   ...
   */
  double calc_distance(CE_Graph_vertex<uint32_t, uint32_t> &v, vid_t pivot, int distance_metric) {
...
struct ItemDistanceProgram : public CE_GraphProgram<VertexDataType, EdgeDataType> {
    ...
    if (gcontext.iteration % 2 == 0) {
      if (adjcontainer->is_pivot(v.id()) && is_item(v.id())){
        ...
        for(int i=0; i<v.num_edges(); i++) {
          CE_Graph_edge<uint32_t> * e = v.edge(i);
...
int main(int argc, const char ** argv) {
 ...
 logstream(LOG_INFO) << "M = " << M << std::endl;
  ...
  CE_Graph_engine<VertexDataType, EdgeDataType> engine(training, 1, true, m); 
