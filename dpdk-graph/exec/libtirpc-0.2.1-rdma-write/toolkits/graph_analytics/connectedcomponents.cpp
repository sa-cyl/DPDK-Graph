 ...
 * of CE_Graph, improving the convergence considerably. Consider
 ...
 * (due to deterministic parallellism of CE_Graph), this does not compromise correctness.
 *
 * @author Aapo Kyrola
 */
...
class OutputVertexCallback : public VCallback<VertexDataType> {
  public:
    ...
    virtual void callback(vid_t vertex_id, VertexDataType &value) {
        fprintf(pfile, "%u 1 %u\n", vertex_id+1, value); //CE_Graph offsets start from zero, while matlab from 1
